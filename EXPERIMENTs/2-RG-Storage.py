...
def test_storage_size(folder_list, tb_list, rg_tb_list):
    original_size_fdb = []
    original_filter_size = []
    addrange_size_fdb = []
    addrange_filter_size = []
    ratio_fdb = []
    ...
    for folder_name in folder_list:
        ...
        ct_spx = spx.Client()
        ct_spx.load_tables(folder_name, rg_tb_list)
        emm = ct_spx.construct_index()
        addrange_size_spx.append(len(pickle.dumps(emm, -1)))
        del emm
...
if __name__ == '__main__':
    ...
    """
    rg_tb_list = [x + "_rg" for x in tb_list]

    original_result = test_storage_size(test_folder, tb_list)
    addrange_result = test_storage_size(test_folder, rg_tb_list)

    ratio_fdb = []
    for x, y in zip(original_result[0], addrange_result[0]):
        ratio_fdb.append(Decimal((y - x) / x).quantize(Decimal("0.00")))

    ratio_spx = []
    for x, y in zip(original_result[2], addrange_result[2]):
        ratio_spx.append(Decimal((y - x) / x).quantize(Decimal("0.00")))
    ...
    data_frame = pd.DataFrame({
        ...
        "FInEDB Range Size": [bytes2MB(x) for x in addrange_result[0]],
        "Filter Range": [bytes2MB(x) for x in addrange_result[1]],
        ...
        "SPX Range Size": [bytes2MB(x) for x in addrange_result[2]],
        "EMM_R Range": [bytes2MB(x) for x in addrange_result[3]],
        "EMM_C Range": [bytes2MB(x) for x in addrange_result[4]],
        "EMM_V Range": [bytes2MB(x) for x in addrange_result[5]],
        "Ratio SPX": ratio_spx
    })
    data_frame.to_csv("./2-RG-Storage_lineitem.csv", index=False)
    print(data_frame)
